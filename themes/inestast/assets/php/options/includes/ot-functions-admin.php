...
if ( ! function_exists( 'ot_layouts_view' ) ) {
  ...
  function ot_layout_view( $key, $smof_data = '', $active_layout = '' ) {
    ...
    <div class="option-tree-setting">
      ...
      <input type="hidden" name="option_tree_layouts[' . esc_attr( $key ) . ']" value="' . $smof_data . '" />
    </div>';
    ...
    echo '<form method="post" id="option-tree-options-layouts-form">';
      ...
      if ( is_array( $layouts ) && count( $layouts ) > 1 ) {
        ...
        foreach( $layouts as $key => $smof_data ) {
          ...
          echo '<input type="hidden" name="option_tree_layouts[' . $key . ']" value="' . ( isset( $smof_data ) ? $smof_data : '' ) . '" />';
