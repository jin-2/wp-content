...
if ( ! function_exists( 'ot_type_export_data' ) ) {
  ...
  function ot_type_export_data() {
    ...
    echo '<div class="format-setting type-textarea simple has-desc">';
      ...
      /* get theme options data */
      $smof_data = get_option( 'option_tree' );
      $smof_data = ! empty( $smof_data ) ? ot_encode( serialize( $smof_data ) ) : '';
        
      echo '<div class="format-setting-inner">';
        echo '<textarea rows="10" cols="40" name="export_data" id="export_data" class="textarea">' . $smof_data . '</textarea>';
      echo '</div>';
      ...
      echo '<div class="format-setting type-textarea has-desc">';
        ...
        echo '<div class="format-setting-inner">';
          ...
          if ( is_array( $layouts ) && ! empty( $layouts ) ) {
            ...
            foreach( $layouts as $key => $smof_data ) {
              ...
              /* content */
              echo '<li class="ui-state-default list-layouts">' . ot_layout_view( $key, $smof_data, $active_layout ) . '</li>';
